...
def export_netCDF(md, filename):  # {{{
    ...
    try:
        ...
        StepNum = 1
    except TypeError:  #this is not a result so no results in there
        StepNum = 0
        ...
        for field in fields:
            ...
            if type(Var) == list:
                ...
                if len(Var) == 0:
                    ...
                    #returns False for exotic types (typically results)
                ...
                if StdList:  # this is a standard or empty list just proceed
                    ...
                    Listsize = len(Var)
                    if group == 'results':  #for results we reshape the data following time rather than subgrouping
            ...
            # this is an issm class
                ...
                if type(md.__dict__[group].__dict__[field]).__name__ == 'solution':
                    #for results we reshape the data following time rather than subgrouping
...
def CreateVar(NCData, var, field, Group, DimDict, *SupDim):  # {{{
    ...
    # treating scalar string or bool as attribute
    ...
    # Now dealing with numeric variables
        ...
        try:
            ...
            if nan_val.all():
                ...
                naned = invar
        except TypeError:  # type does not accept nan, get value of the variable
            naned = invar
...
def GetDim(NCData, val_shape, val_type, DimDict, val_dim):  #{{{
    # ============================================================================
    # retrieve the dimension tuple from a dictionary
    # ============================================================================
    output = []
    if val_type in [collections.OrderedDict, dict]:  # dealing with a dictionary
        try:
            output = [str(DimDict[val_shape])]  # first try to get the corresponding dimension if ti exists
        ...
        for dim in range(0, val_dim):  # loop on the dimensions
            try:
                output = output + [str(DimDict[val_shape[dim]])]  # test if the dimension already exists
